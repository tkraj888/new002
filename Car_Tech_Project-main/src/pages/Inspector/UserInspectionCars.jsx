...
//   const { data, error, isLoading } = useBiddingCarByDealerIdQuery(UserId);
  const {data ,error ,isLoading } = useListbyInspectorIdQuery({inspectorId :UserId ,page: pageNo , size : itemsPerPage})
  const activeCarsData = data?.list?.filter(car => car?.carStatus === "ACTIVE");
  const pendingCarsData = data?.list?.filter(car => car?.carStatus === "pending");
  const sellCarsData = data?.list?.filter(car => car?.carStatus === "SOLD");
  ...
  const [filteredCars, setFilteredCars] = useState(data?.list || []);
...
//  console.log("active",perInspection)
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: "status",
      Cell: (cell) => {
        return  cell.row.values.status == "InProgress" ? (
            ...
            <div className="flex gap-2 justify-center items-center">
                {
                    ...
                    (
                        <Link to={`/inspector/users/car/${cell.row.values.userFormId}`}>
                        <Button variant="gradient" color="green" size="sm">
                            Add Car
                        </Button>
                        </Link>
  ...
  useEffect(() => {
    if (data) {
      ...
      setFilteredCars(data?.list); // Initialize with all cars data
    }
  }, [data]);
            ...
            <Card className="h-full w-full">
              ...
              <div className=" flex items-center justify-between gap-8">
                <div>
                  ...
                  See Information About All sell Cars
                  </Typography>
                </div>
              </div>
            </CardHeader>
